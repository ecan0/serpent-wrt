...
GitHub Actions can also build `.apk` artifacts from official OpenWrt 25.12 SDKs.
The `OpenWrt APK Packages` workflow runs manually or when a GitHub Release is
published; release runs attach the generated packages and their `SHA256SUMS`
files to the release. Manual runs can package a selected ref and optionally
attach the artifacts to an existing release tag.
